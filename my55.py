                ...
                for _, sales_row in matched_sales.iterrows():
                    try:
                        ...
                        if level < 4 and is_company_like(customer_name):
                            ...
                            # 检查是否重复 - 使用现有的direct_sale_df和待添加的next_level_data进行检查
                            is_duplicate = False
                            ...
                            if new_record_key is not None:
                                ...
                                if not is_duplicate_record(next_level_row, direct_sale_df):
                                    ...
                                    if new_record_key not in existing_records_keys:
                                        ...
                                        print(f"添加下一级记录: 级次{level + 1}, 公司:{customer_name}, 产品:{row['产品名称']}, 批号:{row['批号']}")
                                    else:
...
def main():
            ...
            with st.spinner("正在处理数据，请稍候..."):
                ...
                if result_file_path:
                    ...
                    try:
                        ...
                        with col4:
                            total_quantity = result_df['销售数量'].sum()
                            st.metric("总销售数量", f"{total_quantity:,.0f}")
                        ...
                        with pd.ExcelWriter(output, engine='openpyxl') as writer:
                            ...
                            # 添加配置信息工作表
                            config_data = []
                            config_data.append(['类型', '简称/商品名', '全称/规格'])
                            config_data.append(['', '', ''])
    ...
    with st.expander("点击查看详细说明"):
        ...
        **销售明细表：**
        ...
        3. **批号精确匹配**：确保流向数据的准确性
        4. **多级流向生成**：自动识别下游公司，生成多级流向关系
        5. **去重检查**：避免生成重复的出库记录，检查条件包括出库日期、商业公司、产品名称、批号、数量
        
        ### 输出结果
        
        - **流向数据**：包含完整的流向信息，支持最多4级流向
        - **级别统计**：各级别的记录数和销售数量统计
