...
运行命令: streamlit run pharynx_pdf_app.py
"""
...
class PDFReportGenerator:
    ...
    def __init__(self):
        # 注册中文字体
        self.font_name = self.setup_chinese_font()
        print(f"✓ PDFReportGenerator 初始化成功, 使用字体: {self.font_name}")
    ...
    def setup_chinese_font(self):
        ...
        try:
            ...
            if not font_registered:
                ...
                print("  - 建议: 在云端环境添加 packages.txt 文件安装字体")
    ...
    def generate_report(self, result_data, original_image):
        ...
        try:
            ...
            # 标题
            title_text = "Pharynx Health Screening Report" if self.font_name == 'Helvetica' else "正掌讯咽部健康筛查AI分析报告"
            ...
            basic_info = [
                ['Report Time' if self.font_name == 'Helvetica' else '报告生成时间', 
                 result_data['timestamp']],
                ['Diagnosis' if self.font_name == 'Helvetica' else '诊断结果', 
                 f"{classification['disease_name_en']} ({classification['disease_name']})"],
                ['Confidence' if self.font_name == 'Helvetica' else '置信度', 
                 f"{classification['confidence']}%"],
                ['Severity' if self.font_name == 'Helvetica' else '严重程度', 
                 classification['severity_cn']]
            ]
            ...
            # 添加原始图像
            heading_text = "Original Image" if self.font_name == 'Helvetica' else "原始咽部图像"
            ...
            # 特征分析
            heading_text = "Feature Analysis" if self.font_name == 'Helvetica' else "特征分析"
            story.append(Paragraph(heading_text, heading_style))
            ...
            features_data = [
                ['Feature Type' if self.font_name == 'Helvetica' else '特征类型', 
                 'Feature Name' if self.font_name == 'Helvetica' else '特征名称', 
                 'Value' if self.font_name == 'Helvetica' else '数值'],
                ['Color' if self.font_name == 'Helvetica' else '颜色特征', 
                 'Red Ratio' if self.font_name == 'Helvetica' else '红色区域占比', 
                 f"{color_features['red_ratio']:.2%}"],
                ['', 
                 'White Ratio' if self.font_name == 'Helvetica' else '白色区域占比', 
                 f"{color_features['white_ratio']:.2%}"],
                ['', 
                 'Avg Brightness' if self.font_name == 'Helvetica' else '平均亮度', 
                 f"{color_features['avg_brightness']:.1f}"],
                ['Texture' if self.font_name == 'Helvetica' else '纹理特征', 
                 'Edge Density' if self.font_name == 'Helvetica' else '边缘密度', 
                 f"{texture_features['edge_density']:.2%}"],
                ['', 
                 'Texture Variance' if self.font_name == 'Helvetica' else '纹理方差', 
                 f"{texture_features['texture_variance']:.1f}"],
            ]
            ...
            # 症状
            heading_text = "Possible Symptoms" if self.font_name == 'Helvetica' else "可能症状"
            story.append(Paragraph(heading_text, heading_style))
            ...
            # 建议
            heading_text = "Health Recommendations" if self.font_name == 'Helvetica' else "健康建议"
            story.append(Paragraph(heading_text, heading_style))
            ...
            disclaimer_text = "Disclaimer: This report is for reference only and cannot replace professional medical diagnosis."
            if self.font_name != 'Helvetica':
                disclaimer_text = "免责声明: 本报告由AI系统自动生成,仅供健康筛查参考,不能替代专业医疗诊断。如有不适症状,请及时就医咨询专业医生。"
            
            story.append(Paragraph(disclaimer_text, disclaimer_style))
            story.append(Spacer(1, 0.3*cm))
            story.append(Paragraph(
                "© 2025 Pharynx Health Screening System",
                disclaimer_style
            ))
...
def main():
    ...
    try:
        ...
        <div style='text-align: center; color: #888; padding: 20px;'>
            ...
            <p style='margin-top: 10px; font-size: 12px;'>© 2025 咽部健康筛查AI系统 | Powered by AI</p>
        </div>
...
if __name__ == "__main__":
    ...
    main()("# 🏥 咽部健康筛查")
        ...
        <div style='text-align: center;'>
            <h1 style='color: #2E86AB;'>🏥 咽部图像智能筛查系统</h1>
