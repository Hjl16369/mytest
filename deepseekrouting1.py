...
def generate_professional_pdf_report(df, data, path_before, path_after, dist_before, dist_after, 
    ...
    with PdfPages(buffer) as pdf:
        ...
        # 主标题
        ax.text(0.5, 0.75, '正掌讯终端巡店路线优化分析报告',
...
报告版本: 3.0"""
        ...
        # 页脚
        ax.text(0.5, 0.10, '正讯软件 版权所有 © 2026',
        ...
        summary_lines = [
            ...
            "",
            "分析结论:",
            f"本优化方案将 {df.iloc[best_start_idx]['Name']} 确定为最优起点。"
            # 已删除"建议立即实施此优化路线方案。"
        ]
...
平均药店间距: {np.mean(dist_matrix[np.triu_indices(len(df), k=1)]):.2f} km"""
        ...
        # 创建表格（根据要求调整列宽）
        # 调整列宽：缩短"巡店顺序"和"原表格序号"，加大"药店名称"
        col_widths = [0.05, 0.45, 0.08, 0.20, 0.20]  # 调整后的列宽比例
        ...
        for i in range(1, len(table_data)):
            for j in range(len(display_route.columns)):
                ...
                if i == 1:  # 起点
                    ...
                    cell.set_text_props(weight='bold', fontsize=7, color='#1B5E20')
                elif i == len(table_data) - 1 and not show_truncated:  # 终点
                    cell.set_facecolor('#FFEBEE')
                    cell.set_text_props(weight='bold', fontsize=7, color='#C62828')
                else:
        ...
        # 如果表格被截断，添加说明
        if show_truncated:
            ax.text(0.05, 0.04, f"注: 显示前25条记录，共{len(route_df)}家药店（完整列表请参考CSV文件）",
...
报告生成时间: {datetime.now().strftime('%Y年%m月%d日 %H:%M')}"""
        ...
        ax.text(0.5, 0.45, disclaimer_text,
        ...
        contact_text = """联系方式
西安正讯软件有限公司
地址: 西安市高新技术产业开发区高新二路4号
咨询热线: 17791397170
邮箱: jacensoft@163.com
官网: www.jacensoft.cn"""
        
        # 创建一个居中的文本框
        ax.text(0.5, 0.25, contact_text,
               ha='center', va='center', fontsize=9,
               color=SECONDARY_COLOR,
               linespacing=1.8,
