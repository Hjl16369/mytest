    ...
    try:
        ...
        if missing_columns:
            ...
            if start_convert and api_key:
                ...
                st.write(f"**调试信息：成功转换的记录数：{len(success_df)}**")
                
                if len(success_df) > 0:
                    st.markdown("---")
                    st.subheader("🗺️ 客户位置地图展示（前10个成功转换的客户）")
                    
                    # 获取前10个成功的客户，并确保经纬度不为空
                    map_df = success_df.head(10).copy()
                    
                    # 过滤掉经纬度为空的记录
                    map_df = map_df[map_df["经度"].notna() & map_df["纬度"].notna()]
                    map_df = map_df[map_df["经度"] != ""]
                    
                    st.write(f"**调试信息：有效坐标的记录数：{len(map_df)}**")
                    st.write("**前3条记录的经纬度：**")
                    st.write(map_df[["客户名称", "经度", "纬度"]].head(3))
                    ...
                    if len(map_df) > 0:
                        try:
                            ...
                            if len(map_df) == 0:
                                st.error("❌ 没有有效的经纬度数据可以显示在地图上")
                            else:
                                # 计算地图中心点（所有客户的平均位置）
                                center_lat = float(map_df["纬度"].mean())
                                center_lng = float(map_df["经度"].mean())
                                
                                st.write(f"**地图中心坐标：经度 {center_lng:.6f}, 纬度 {center_lat:.6f}**")
                                ...
                                st.write(f"**成功添加 {marker_count} 个标记点**")
        ...
        st.info("请确保上传的是有效的Excel文件，并包含所有必需的列。")
