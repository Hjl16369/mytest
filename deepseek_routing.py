...
def generate_professional_pdf_report(df, data, path_before, path_after, dist_before, dist_after, 
    ...
    # 配置字体 - 确保中文字体支持
    plt.rcParams.update({
        'font.sans-serif': ['SimHei', 'DejaVu Sans', 'Arial', 'Helvetica'],
...
报告版本: 3.0"""
        ...
        # 指标1: 原始路径
        ax.text(0.2, metrics_y, f'{dist_before:.2f} km',
               ha='center', va='center', fontsize=14, fontweight='bold',
               color=SECONDARY_COLOR, transform=fig.transFigure)
        ax.text(0.2, metrics_y - 0.05, '原始路径长度',
               ha='center', va='top', fontsize=10,
               color=SECONDARY_COLOR, transform=fig.transFigure)
        
        # 指标2: 优化后路径
        ax.text(0.4, metrics_y, f'{dist_after:.2f} km',
               ha='center', va='center', fontsize=14, fontweight='bold',
               color=SUCCESS_COLOR, transform=fig.transFigure)
        ax.text(0.4, metrics_y - 0.05, '优化后路径长度',
               ha='center', va='top', fontsize=10,
               color=SECONDARY_COLOR, transform=fig.transFigure)
        
        # 指标3: 节省距离
        ax.text(0.6, metrics_y, f'{dist_before - dist_after:.2f} km',
               ha='center', va='center', fontsize=14, fontweight='bold',
               color=HIGHLIGHT_COLOR, transform=fig.transFigure)
        ax.text(0.6, metrics_y - 0.05, '节省距离',
               ha='center', va='top', fontsize=10,
               color=SECONDARY_COLOR, transform=fig.transFigure)
        
        # 指标4: 优化比例
        savings_percent = ((dist_before - dist_after) / dist_before * 100) if dist_before > 0 else 0
        ax.text(0.8, metrics_y, f'{savings_percent:.1f}%',
               ha='center', va='center', fontsize=14, fontweight='bold',
               color=ACCENT_COLOR, transform=fig.transFigure)
        ax.text(0.8, metrics_y - 0.05, '优化比例',
               ha='center', va='top', fontsize=10,
               color=SECONDARY_COLOR, transform=fig.transFigure)
        ...
        # 摘要文本
        summary_text = f"""本报告对 {len(df)} 家药店的巡店路线进行了深度优化分析。通过先进的路径优化算法，
我们识别出全局最优的巡店顺序，显著降低了总行驶距离，提升了巡店效率。

核心发现：
• 最优起点：{df.iloc[best_start_idx]["Name"]}
• 优化后路径总长度：{dist_after:.2f} km
• 相比原始路线节省：{dist_before - dist_after:.2f} km ({savings_percent:.1f}%)
...
4. 考虑交通因素进行时段化路线规划"""
        ...
        # 添加摘要背景框
        summary_rect = Rectangle((0.05, 0.10), 0.9, 0.80, 
                                facecolor='#F8F9FA', edgecolor=PRIMARY_COLOR,
        ...
        # 标题
        fig.suptitle('路线优化对比分析', fontsize=18, fontweight='bold', color=PRIMARY_COLOR, y=0.97)
        ...
        ax3.set_ylabel('距离 (km)', fontsize=10)
        ax3.set_title('路线长度对比', fontsize=12, fontweight='bold', pad=10)
...
最长药店间距: {np.max(dist_matrix[np.triu_indices(len(df), k=1)]):.2f} km"""
        ...
        # 添加指标背景框
        metrics_rect = Rectangle((0.1, 0.1), 0.8, 0.8, 
                                facecolor='#F8F9FA', edgecolor=ACCENT_COLOR,
                                linewidth=1.5, transform=ax4.transAxes,
                                alpha=0.3, zorder=1)
        ax4.add_patch(metrics_rect)
        
        ax4.text(0.5, 0.8, '关键指标摘要',
                ha='center', va='center', fontsize=12, fontweight='bold',
                color=PRIMARY_COLOR, transform=ax4.transAxes)
        ...
        # 创建带专业样式的表格
        col_widths = [0.08, 0.35, 0.12, 0.22, 0.22]
        ...
        # 如果表格被截断，添加说明
        if show_truncated:
            trunc_note = f"注: 此处显示前25条记录，共{len(route_df)}家药店（完整列表请参考CSV文件）"
...
• 优化后总距离: {dist_after:.2f} km
• 平均药店间距: {np.mean(dist_matrix[np.triu_indices(len(df), k=1)]):.2f} km
• 路线非直线系数: {dist_after / dist_matrix[path_after[0], path_after[-1]]:.2f if dist_matrix[path_after[0], path_after[-1]] > 0 else 'N/A'}"""
        
        # 添加统计摘要背景框
        stats_rect = Rectangle((0.65, 0.20), 0.3, 0.15, 
                              facecolor='#F8F9FA', edgecolor=SECONDARY_COLOR,
...
• 精度: 优于0.1%"""
        ...
        # 添加算法说明背景框
        algo_rect = Rectangle((0.05, 0.10), 0.45, 0.80, 
                             facecolor='#F8F9FA', edgecolor=PRIMARY_COLOR,
                             linewidth=1.5, transform=fig.transFigure,
                             alpha=0.3, zorder=1)
        ax.add_patch(algo_rect)
        
        ax.text(0.10, 0.85, algo_text,
               ha='left', va='top', fontsize=9,
               color=SECONDARY_COLOR,
               transform=fig.transFigure,
               linespacing=1.5)
...
官网: www.zzxtech.com"""
        ...
        # 添加实施建议背景框
        imp_rect = Rectangle((0.52, 0.10), 0.45, 0.80, 
                            facecolor='#F8F9FA', edgecolor=ACCENT_COLOR,
                            linewidth=1.5, transform=fig.transFigure,
                            alpha=0.3, zorder=1)
        ax.add_patch(imp_rect)
        
        ax.text(0.57, 0.85, implementation_text,
               ha='left', va='top', fontsize=9,
               color=SECONDARY_COLOR,
               transform=fig.transFigure,
               linespacing=1.5)
...
报告生成时间: {}""".format(datetime.now().strftime('%Y年%m月%d日 %H:%M'))
        
        disclaimer_rect = Rectangle((0.05, 0.02), 0.90, 0.06, 
                                   facecolor='#FFF3E0', edgecolor='#FF9800',
                                   linewidth=1, transform=fig.transFigure,
                                   alpha=0.5, zorder=1)
        ax.add_patch(disclaimer_rect)
        
        ax.text(0.5, 0.05, disclaimer_text,
               ha='center', va='center', fontsize=8,
               color='#E65100',
               transform=fig.transFigure)
